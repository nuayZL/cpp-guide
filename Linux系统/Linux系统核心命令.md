...
![image-20210705183457018](C:\Users\Echo\AppData\Roaming\Typora\typora-user-images\image-20210705183457018.png)
...
```
Linux系统中一切皆文件。
普通文件、目录、字符设备、块设备和网络设备(套接字)等Unix/Linux都被当做文件来对待
```



### 文件属性操作（用户组、拥有者、权限、用户掩码、测试）
...
### 文件整体操作（展开、复制、删除、移动、修改时间属性、辨识文件类型、查找二进制及源代码文件、查找文件）
####  lsls命令用于显示指定工作目录下之内容
##### 功能
...
### 文件内容操作（文件查看、反向输出、显示行号、翻页显示、前翻、只看前/后几行、压缩、分段处理、对比差异）
####  cat文本文件查看和连接工具
##### 功能
 由第一行开始显示文件内容
 用于连接文件并打印到标准输出设备上
...
```
是一种编程语言、支持条件判断、数组、循环等功能。可以将其看成脚本语言解释器
```
...
# 性能检测相关命令（动态显示、定期显示、显示核的状态、显示内存情况、显示磁盘情况）
...
### VMstat定期检测、虚拟内存性能检测、CPU监测

```
主要用于查看虚拟内存、也可以查看系统其他资源的使用情况、

检测的内容包括CPU使用率、内存使用、虚拟内存交换情况、IO读写情况
```

```
需要指定两个参数  分贝是采样的时间间隔和采样的次数
```
...
```
shift+'c'   可以将进程按照CPU使用率从大到小进行排序。
shift+'p'  可以将进程按照内存使用率从大到小进行排序。
```
...
### kill发送信号到指定进程来杀死单个或多个进程
#### 功能
发送信号到指定进程

#### 常用的九种信号（中断、退出、终止、强制终止、继续、暂停）
...
### ulimit 限制用户使用系统的某些资源 包括可以开启的文件数量， 可以使用的 CPU 时间，可以使用的内存总量等等
#### 功能
...
2.为什么会有管线？bash命令运行的时候有输出的数据会出现。这些数据有时候需要经过处理变成我们需要的格式、这就需要管线命令（pipe）

3.举例  ls -al /etc | less
上述使得ls命令输出后的内容、能够被less读取，并且使用less的功能，来翻动相关的信息。
```

### cut将『同一行里面的数据进行分解！』  处理的讯息是以『行』为单位
...
### grep（过滤功能）（匹配正则表达式）分析一行讯息， 若当中有我们所需要的信息，就将该行拿出来～
